...
export default function GenerateContract() {
  ...
  const generateContractPDF = () => {
    ...
    const ownerReg = ownerDetails.reg;
    const ownerFiscal = ownerDetails.fiscal;
    const ownerAddress = ownerDetails.address;
    const ownerCounty = ownerDetails.county;
    const ownerCity = ownerDetails.city;
    const ownerBank = ownerDetails.bank;
    const ownerBankName = ownerDetails.bankName;
    const ownerRepresentative = ownerDetails.representative;
    const ownerEmail = ownerDetails.email;
    const ownerPhone = ownerDetails.phone;

    const tenantName = tenantDetails.name;
    const tenantReg = tenantDetails.reg;
    const tenantFiscal = tenantDetails.fiscal;
    const tenantAddress = tenantDetails.address;
    const tenantCounty = tenantDetails.county;
    const tenantCity = tenantDetails.city;
    const tenantBank = tenantDetails.bank;
    const tenantBankName = tenantDetails.bankName;
    const tenantRepresentative = tenantDetails.representative;
    const tenantEmail = tenantDetails.email;
    const tenantPhone = tenantDetails.phone;

    const propertyAddress = propertyDetails.address;
    const propertyRooms = propertyDetails.rooms;
    const rentAmount = propertyDetails.rentAmount;
    const vatIncluded = propertyDetails.vatIncluded;
    const paymentDay = propertyDetails.paymentDay;
    const lateFee = propertyDetails.lateFee;
    const contractDuration = propertyDetails.contractDuration;
    const renewalPeriod = propertyDetails.renewalPeriod;
    const unilateralNotice = propertyDetails.unilateralNotice;
    const terminationNotice = propertyDetails.terminationNotice;
    const earlyTerminationFee = propertyDetails.earlyTerminationFee;
    const latePaymentTermination = propertyDetails.latePaymentTermination;
    const securityDeposit = propertyDetails.securityDeposit;
    const depositReturnPeriod = propertyDetails.depositReturnPeriod;

    const waterColdMeter = utilities.waterCold;
    const waterHotMeter = utilities.waterHot;
    const electricityMeter = utilities.electricity;
    const gasMeter = utilities.gas;
    const startDate = validFrom;

    const ownerSignatureDate = format(new Date(), 'dd.MM.yyyy');
    const ownerSignatureName = ownerDetails.name;
    const tenantSignatureDate = format(new Date(), 'dd.MM.yyyy');
    const tenantSignatureName = tenantDetails.name;
