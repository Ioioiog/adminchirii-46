...
export function MessageList({
    ...
    const handleScroll = (event: Event) => {
      ...
      if (scrollTop < 50 && messages && messages.length > 0) {
        ...
        if (currentFirstMessageIndex > 0) {
          const newMessages = messages.slice(Math.max(0, currentFirstMessageIndex - 12), currentFirstMessageIndex + visibleMessages.length);
  ...
  return <div className="flex-1 flex flex-col overflow-hidden">
      <ScrollArea ref={scrollViewportRef} className={className || "flex-1 h-full"}>
        <div className="space-y-4 p-4 min-h-full transition-colors duration-200 ease-in-out bg-slate-50">
